...
# 模型保存回调函数
check_point = keras.callbacks.ModelCheckpoint("model-v6.hdf5", save_best_only=True, period=5)
# input:
...
model.fit(x_train, [y_train, y_train_pro], validation_data=(x_val, [y_val, y_val_pro]),
          batch_size=32, epochs=200, verbose=1,
          callbacks=[eval_callback, lr_callback,check_point])
